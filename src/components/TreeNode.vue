  ...
  <div>
    <div class="mb-1">
      <div class="rubric">
        <input
          ...
          type="checkbox"
          v-model="isSelected"
          @click="handleRubricClick"
        />
        <a class="form-check-label" :href="rubric.url">{{ rubric.title }}</a>
        ({{ rubric.count }},
        {{ totalCount == 0 ? 0 : totalCount + rubric.count }})
      </div>
    </div>
    <div class="mb-1 ps-3" v-show="rubric && rubric.children && isSelected">
      <tree-node
        ...
        :key="child.id"
        :rubric="child"
        :show-empty-rubrics="showEmptyRubrics"
